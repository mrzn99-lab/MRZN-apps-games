...
async function loadApps() {
  ...
  if (error) {
    document.getElementById("app-grid-wrap").innerHTML =
      `<div class="empty-state">অ্যাপ লোড করা যায়নি। Supabase কনফিগারেশন চেক করুন।</div>`;
    console.error(error);
    return;
  }
...
function buildCategoryChips() {
  ...
  wrap.innerHTML = cats.map(c =>
    `<button class="filter-chip ${c === "all" ? "active" : ""}" data-cat="${escapeHTML(c)}">${c === "all" ? "সব" : escapeHTML(c)}</button>`
  ).join("");
...
function renderApps(list) {
  ...
  if (!list.length) {
    wrap.innerHTML = `<div class="empty-state">
      ...
      <div>কোনো অ্যাপ পাওয়া যায়নি।</div>
    </div>`;
    return;
  }
